...
const Create = (options, emit, toggle) => {
    if (options.editable) {
        ...
        const handleChange = (e) => {
            options.editable.dataset.offset = getCaretPosition(
                options.editable
            );
        };
    ...
    if (options.frequent == true) {
        ...
        frequentTitle.classList.add(
            options.classnames.category,
            options.classnames.frequentTitle
        );
    ...
    if (options.trigger && options.tether) {
        ...
        if (placements.indexOf(options.placement) == -1) {
            throw new Error(
                `Invalid attachment '${
                    options.placement
                }'. Valid placements are '${placements.join(`', '`)}'.`
            );
        }
        ...
        switch (options.placement) {
            ...
            case placements[2]:
                attachment =
                    (options.placement == placements[0]
                        ? placements[2]
                        : placements[0]) + " center";
                targetAttachment =
                    (options.placement == placements[0]
            ...
            case placements[3]:
                attachment =
                    "top " +
                    (options.placement == placements[1]
                        ? placements[3]
                        : placements[1]);
                targetAttachment =
                    "top " +
