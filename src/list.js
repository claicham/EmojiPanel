...
const list = (options, panel, json, emit) => {
    ...
    const searchTitle = panel.querySelector(
        "." + options.classnames.searchTitle
    );
    const frequentResults = panel.querySelector(
        "." + options.classnames.frequentResults
    );
    const frequentResultsWrapper = frequentResults.querySelector("div");
    ...
    if (options.search == true) {
        searchInput.addEventListener("input", (e) => {
            const emojis = results.querySelectorAll(
                "." + options.classnames.emoji
            );
            const titles = results.querySelectorAll(
                "." + options.classnames.category
            );
    ...
    Object.keys(json).forEach((i) => {
        ...
        if (
            options.hidden_categories.indexOf(category.name) > -1 ||
    ...
    if (options.fitzpatrick) {
        ...
        modifierToggle.classList.add(
            options.classnames.btnModifier,
            options.classnames.btnModifierToggle,
            options.classnames.emoji
        );
        ...
        Object.keys(modifiers).forEach((m) => {
            ...
            modifierBtn.addEventListener("click", (e) => {
                ...
                const emojis = [].forEach.call(
                    options.container.querySelectorAll(
                        `.${options.classnames.results}  .${options.classnames.emoji}`
                    ),
                    (emoji) => {
                        if (emoji.dataset.fitzpatrick) {
                            ...
                            emoji.parentNode.replaceChild(
                                Emojis.createButton(emojiObj, options, emit),
                                emoji
                            );
                        }
                    }
