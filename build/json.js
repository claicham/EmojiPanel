...
module.exports = () => {
    ...
    let emojis = JSON.parse(
        fs.readFileSync("node_modules/emojione/emoji.json")
    );
    let emojisLength = 0;
    ...
    emojis = _.chain(emojis)
        .each((emoji, name) => {
            ...
            if (emoji.unicode_alt.length > 1) {
                emoji.char = emojione.convert(emoji.unicode_alt);
            } else {
            ...
            for (let m in modifiers) {
                ...
                if (index > -1) {
                    ...
                    if (modifierEmojis.indexOf(firstCode) == -1) {
                        const mainEmoji = _.find(
                            emojis,
                            (e) => e.unicode == firstCode
                        );
                        if (mainEmoji) {
        ...
        .map((emojis, name) => {
            // Turn each category into { name, emojis }
        ...
        .reject(
            (category) => typeof categoryIcons[category.name] == "undefined"
        )
    ...
    _.each(emojis._wrapped, (category) => {
        console.log(
            "-- " +
                Object.keys(category.emojis).length +
                "\t" +
                (category.emojis[0].char ? category.emojis[0].char : " ") +
                "  " +
                category.name
        );
    });
