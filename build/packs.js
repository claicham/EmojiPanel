...
const packs = [
    {
        ...
        findFile: (unicode) => `build/twemoji/assets/svg/${unicode}.svg`,
    },
    {
        name: "emojione",
        location: "node_modules/emojione",
        findFile: (unicode) =>
            `node_modules/emojione/assets/svg/${unicode}.svg`,
    },
];
...
const downloadTwemoji = () => {
    ghdownload(
        { user: "twitter", repo: "twemoji", ref: "master" },
        process.cwd() + "/build/twemoji"
    ).on("end", function () {
        buildNext();
    });
};
